...
describe("Receiving Payload", function () {

    it("must process the payload", function (done) {
        request.post({ url: baseUrl, json: payloadRequest }, function (error, response, body) {

            expect(response.statusCode).toBe(200);            

            expect(body).toEqual(payloadResponse);
    ...
    it("invalid JSON, return a JSON response with HTTP status 400 Bad Request", function (done) {
        request.post(baseUrl, "invalidJson: '' ", function (error, response, body) {

            expect(response.statusCode).toBe(400);

            expect(body).toEqual('{"error":"Could not decode request: JSON parsing failed"}');
